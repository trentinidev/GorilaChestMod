...
| File | Use it for |
| --- | --- |
| `GorilaChestMod-2.3.0-thunderstore.zip` | Thunderstore upload, and `Import local mod` in r2modman or Thunderstore Mod Manager. The dll sits at the root. |
| `GorilaChestMod-2.3.0-nexus.zip` | **Dedicated server and manual installs.** It already has the `BepInEx/plugins/GorilaChestMod/` layout, so it extracts straight over the game or server folder. |
